...
// ===== CONEXÃO RX -> TX COM LATCHING =====
// Quando RX termina, levanta tx_send
// Quando TX termina, desce tx_send
reg tx_send_latch;
...
always @(posedge clk or negedge rst) begin
    if (!rst) begin
        ...
        if (rx_done) begin
            tx_send_latch <= 1'b1;  // Levantar quando RX completa
        end else if (tx_done) begin
            tx_send_latch <= 1'b0;  // Descer quando TX completa
        end
    end
end

assign tx_send = tx_send_latch;
